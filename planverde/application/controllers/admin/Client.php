    ...
    public function clientList($type = null){
        if ($this->input->is_ajax_request()) {
            ...
            foreach ($fetch_data as $_key => $client_details) {
                ...
                if (!empty($deleted)) {
                    $action .= '<button type="button" data-toggle="tooltip" data-placement="top" class="btn btn-danger btn-xs" title="Click para Eliminar " onclick="deleteClient('.$client_details->client_id.')"><span class="fa fa-trash-o"></span></button>' . ' ';
                }
...
//                }
    ...
    public function delete_client($client_id, $yes = null, $bulk = null){
        $deleted = can_action('4', 'deleted');
        $sbtn = $this->input->post('submit', true);
        if (!empty($bulk)) {
            $sbtn = true;
...
//            messages for user
    ...
    public function delete_client($id = NULL){
        if(isset($id)){
            ...
            $data_document = $this->db->where('client_id', $id)->get('tbl_client')->row();
            if(count($data_document) > 0){
                if($this->db->where('client_id', $id)->delete('tbl_client')){
                    $data = [
                        'type'    => 'success',
