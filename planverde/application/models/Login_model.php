...
class Login_Model extends MY_Model{
    ...
    public function login(){        
        ...
        if (!empty($getUser) && $getUser->activated == 1 && $getUser->banned == 0){
                ...
                if ($getUser->role_id != '2'){
                    ...
                    if (!empty($mark_attendance)){
                        ...
                        if (!empty($clocking)){
                            ...
                            $this->set_clocking(0, $getUser->user_id, true, true);
                    ...
                    $data = array(
                        ...
                        'user_id' => $getUser->user_id,
                        'last_login' => $getUser->last_login, // PARA LA SECCIÓN "Último acceso"
                        'online_time' => time(),
                        'loggedin' => TRUE,
                        'user_type' => 2,
                        'user_flag' => 2,
            ...
            $data = array(
                'c_message' => lang('email_or_usernme_error'),
                'message' => 'error'
            );
            $this->session->set_userdata($data); // SETEAR EN $this->session->userdata();
        }
