...
class Settings extends Admin_Controller{
  ...
  public function index(){
    ...
    $data['title'] = 'Ajustes de sistema | PLAN VERDE';
    $can_do = can_do(111);
    if(!empty($can_do)){
  ...
  public function form_change_password(){
    ...
    $data['subview'] = $this->load->view('admin/form_change_pass', $data);
    $this->load->view('admin/_layout_modal', $data);
  }
  public function set_password_aQ(){
    $sess_userType = $this->session->userdata('user_type');
    $sess_userId = $this->session->userdata('user_id');
    if($sess_userType == 1){
      $checkUserType = $this->admin_model->check_by(array('role_id' => $sess_userType), 'tbl_users');
      if(!empty($checkUserType)){
        ...
        $this->settings_model->save($data, $sess_userId);
        $type = "success";
        $message = '<strong>Contraseña actualizada</strong>. Por favor reinicie su sesión';
      }else{
        $type = "error";
      ...
      $type = "error";
      $message = 'Error el usuario no está autorizado para actualizar sus credenciales.';
    }
  ...
  public function save_settings(){
    $input_data = $this->settings_model->array_from_post(array(
      ...
      'company_vat',
      'company_ruc',
      'socialnetwork_facebook_url',
      'socialnetwork_instagram_url',
      'socialnetwork_linkedin_url',        
      'socialnetwork_whatsapp_telephonenumber',
      'socialnetwork_whatsapp_textsendmessage',
      )
    );
  ...
  public function save_theme(){
    ...
    if(!empty($_FILES['company_logo']['name'])){
      $val = $this->settings_model->uploadImage_companylogo('company_logo');
      $val == TRUE || redirect('admin/settings/theme');
      $input_data['company_logo'] = $val['path'];
    }
    if(!empty($_FILES['favicon']['name'])){
      $val = $this->settings_model->uploadImage_favicon('favicon');
      $val == TRUE || redirect('admin/settings/theme');
      $input_data['favicon'] = $val['path'];
    }
    if(!empty($_FILES['login_background']['name'])){
      $val = $this->settings_model->uploadImage_bgcklogin('login_background');
      $val == TRUE || redirect('admin/settings/theme');
  ...
  public function dashboard(){
    ...
    $data['title'] = lang('dashboard_settings');
    $can_do = can_do(145);
    if(!empty($can_do)){
      $data['subview'] = $this->load->view('admin/settings/settings', $data, TRUE);
    }else{
  ...
  public function save_custom_field($id = null){
  ...
  public function change_field_status($id = null){
