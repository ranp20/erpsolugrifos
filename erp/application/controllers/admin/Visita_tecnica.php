...
class Visita_Tecnica extends Admin_Controller
{
  ...
  public function save_visita($id = null)
  {
    ...
    if ($id_visita) {
      ...
      $message = '<span class"n-cont__custom">
                    <span>Nueva valorizacion de <span style="color:#FF0000;">Visita Técnica</span> ingresada,</span>
  ...
  public function visita_tecnica_list($type = NULL)
  {
    ...
    if ($this->input->is_ajax_request()) {
      ...
      $this->datatables->order = array('vt.visita_tecnica_id' => 'desc');
      $where = [];
      if ($this->db->where(['designations_id' => $this->session->userdata('designations_id')])->get('tbl_designations')->row()->departments_id == 2) :
        $where = ['vt.area_inicio' => $this->session->userdata('designations_id')];
      endif;
      if (!empty($type)) {
        $where['vt.status'] = $type;
      } else {
        // $where = null;
      }
      ...
      foreach ($fetch_data as $_key => $visita) {
        ...
        $sub_array[] = display_money($visita->monto);
  ...
  public function report_orden_list($type = NULL)
  {
    ...
    if ($this->input->is_ajax_request()) {
      $this->load->model('datatables');
      $this->datatables->select = 'ser.service,  se.sede, se.direccion, cli.razon_social, vto.*, vt.*, d.*';
      $this->datatables->table = 'tbl_visita_tecnica vt';
      $this->datatables->join_table = array('tbl_services ser', 'tbl_cliente cli', 'tbl_sedes se', 'tbl_visita_tecnica_orden vto', 'tbl_designations d');

      $this->datatables->join_where = array('ser.service_id=vt.service_id', 'vt.cliente_id = cli.cliente_id', 'se.sede_id = vt.sede_id', 'vt.visita_tecnica_id = vto.visita_tecnica_id', 'vto.designation_id_to = d.designations_id');
      ...
      $where = array('vto.designation_id_to' => $this->session->userdata('designations_id'));
      /* print_r( $where);
      exit(); */
      if (!empty($type)) {
        // $where = array('vt.status' => $type);
        array_push($where, array('vt.status' => $type));
      } else {
        // $where = '';
      }
      /* print_r( $where );
