...
class Factura extends Admin_Controller
{
  ...
  public function FacturaList($actionURL = NULL, $type = null)
  {
    ...
    if ($this->input->is_ajax_request()) {
      ...
      $this->datatables->select = '*, f.monto, f.fecha_emision, f.fecha_vencimiento';
      $this->datatables->table = 'tbl_facturas f';
      $this->datatables->join_table = array('tbl_cotizacion_pago cp', 'tbl_cotizaciones c', 'tbl_cliente cli', 'tbl_sedes se', 'tbl_services ser');
      $this->datatables->join_where = array('f.cotizacion_pago_id = cp.cotizacion_pago_id', 'cp.cotizacion_id = c.cotizacion_id', 'c.cliente_id = cli.cliente_id', 'c.sede_id = se.sede_id', 'c.service_id = ser.service_id');
