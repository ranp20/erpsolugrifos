...
function getIdMainFolder($section = NULL){
    if($section != ""){
        if($section == "anuncios"){
            ...
            return '1Wg8VtjI6DkT49ZyJlMp3Bw4LTTD1Ey0E';
        }else{
...
function driveService(){
    ...
    try{
        // ----------------- CREDENCIAL ANTERIOR (ANTERIOR DESARROLLADOR)
        // putenv('GOOGLE_APPLICATION_CREDENTIALS=plan-verde-308823-429d0e20a5db.json');
        // putenv('GOOGLE_APPLICATION_CREDENTIALS=cargardocumentosplanverde-e296d7be81da.json'); // CREDECIALES - 26/11/2023 (plataforma@solugrifos.com)
        putenv('GOOGLE_APPLICATION_CREDENTIALS=subidadedocumentos-85ed8fc49ddd.json'); // CREDECIALES - 09/04/2024 (planverdeplataforma@gmail.com)
        $client = new Google_Client();
        $client->useApplicationDefaultCredentials();
        $client->setScopes(['https://www.googleapis.com/auth/drive.file']);
        return $service = new Google_Service_Drive($client);
    }catch(Google_Service_Exception $error){
        return ["estado"=>false,"error"=>$error->getMessage()];
    }
}
// ------------- CREAR CARPETA - GOOGLE DRIVE
function driveCreate($name, $parent = null, $description = null, $mime_type = 'application/vnd.google-apps.folder'){
    try{
        ...
        $params = ['fields' => 'id', 'supportsAllDrives' => true, 'uploadType' => 'multipart'];
        $createdFile = $service->files->create($file, $params);
        return $createdFile;
    }catch(Google_Service_Exception $error){
        return array("estado"=>false,"error"=>$error->getMessage());
    }
}
// ------------- ACTUALIZAR NOMBRE DE CARPETA - GOOGLE DRIVE
function driveUpdate($id, $newName, $newdescription = '', $mimeType = 'application/vnd.google-apps.folder'){
    try{
        $service = driveService();
        $file = $service->files->get($id, array('fields' => 'name, description'));
        $file->setName($newName);
        if(!is_null($newdescription)){
            $file->setDescription($newdescription);
        }
        $file->setMimeType($mimeType);
        $updatedFile = $service->files->update($id, $file, array('fields' => 'name, description'));
        return $updatedFile;
    }catch(Google_Service_Exception $error){
        return array("estado"=>false,"error"=>$error->getMessage());
    }   
...
function driveDelete($id){
    try{
        ...
        return ["estado" => true,"mensaje" => "Eliminada correctamente"];
    }catch(Exception $error){
        return ["estado" => false,"mensaje" => "No se pudo eliminar la carpeta","error" => $error->getMessage(),];
    }
}
// ------------- SUBIR ARCHIVO - GOOGLE DRIVE
function uploadFileToDrive($name,$document,$description,$parentFolder){
    try{
        $service = driveService();
        $file = new Google_Service_Drive_DriveFile();
        $file->setName($name);
        $tipo_mime = mime_type($name);
        $file->setParents([$parentFolder]);
        $file->setDescription($description);
        $file->setMimeType($tipo_mime);
        $params = [
            'data' => file_get_contents($document),
            'mimeType' => $tipo_mime,
            'uploadType' => 'media',
        ];
        $newFile = $service->files->create($file,$params);
        return $newFile;
    }catch(Google_Service_Exception $error){
        return array("estado"=>false,"error"=>$error->getMessage()); 
    }
