...
class Admin_Model extends MY_Model{
    ...
    public function select_menu_by_id($id, $menuId, $table = null){
        if(empty($table)){
            $table = 'tbl_menu';
        }
    ...
    public function select_menu_first_parent($lastId, $table){
    ...
    public function calculate_amount($year, $month){
        $amount = $this->db->select_sum('amount')->where(array('month_paid' => $month, 'year_paid' => $year))->get('tbl_payments')->row()->amount;
        return ($amount > 0) ? $amount : 0;
    }
    public function check_user_name($user_name, $user_id = null){
