  ...
  public function pvActivitieRealizarList($type = NULL)
  {
    if ($this->input->is_ajax_request()) {
      ...
      if (!empty($type)) {
        ...
        $where = array( 'tbl_pv_order_activities.area_asignada' => $this->session->userdata('designations_id') );
      }
      ...
      foreach ($fetch_data as $_key => $pv_activitie) {
        if( in_array( $pv_activitie->status, [2,3] ) ){
          ...
          $brn_update =  '<span data-placement="top" data-toggle="tooltip" title="EDITAR ACTIVIDAD"><a  data-toggle="modal" data-target="#myModal"  class="btn btn-primary btn-xs"  href="' . base_url() . 'admin/pv_actividades_realizar/add_pv_activitie/' . $pv_activitie->pv_order_activitie_id . '"><span class="fa fa-pencil"></span></a></span>';
