...
class User extends Admin_Controller
{
    ...
    public function userList($filterBy = null)
    {
        if ($this->input->is_ajax_request()) {
            ...
            /* AQMIGUEL NO INCLUIMOS A LOS CLIENTES  */
            $this->db->where_not_in('tbl_users.role_id', 2);
            ...
            foreach ($fetch_data as $_key => $v_user) {
                ...
                if (!empty($account_info)) {
                    ...
                    $sub_array[] = '<strong>' . $v_user->fullname . '</strong>';;
          ...
          title="' . $account_info->ban_reason . '">' . lang('banned') . '</span>';
                    ...
                    $sub_array[] = $role;
                    $rol = $this->db->where( 'designations_id', $v_user->designations_id )->get( 'tbl_designations' )->row();
$sub_array[] = $rol->designations;
                    ...
                    if ($v_user->user_id != $this->session->userdata('user_id')) {
                        if (!empty($can_edit) && !empty($edited)) {
                            /* AQMIGUEL :: QUITANDO EL BOTON DE BANEO */
                            /* if ($account_info->banned == 1) {
                        ...
                        /* AQMUGUEL BLOQUEANDO EL ENVIAR CORREO  */
    ...
    public function save_user($id = null)
    {
        ...
        if (!empty($created) || !empty($edited) && !empty($id)) {
            ...
            if (!empty($check_user) || !empty($check_email)) { // if input data already exist show error alert
                ...
                /* AQMIGUEL :: ASIGNAMOS LS PERMISOS COPLETS DEL GRUPO  */
                $permission = 'everyone';
                if (!empty($permission)) {
