...
class Pv_order extends Admin_Controller
{
  ...
  {
    $data['title'] = 'Ordenes para trabajos';
    $data['page'] = 'Ordenes para trabajos';
  ...
  public function add_pv_cliente($id = NULL)
  {
    $data['title'] = 'Nueva Orden';
    //$data['sedes'] = ($cliente_id = $data['pv_order_info']->cliente_id) ? $this->db->where(['cliente_id' => $cliente_id])->get('tbl_sedes')->result() : '';
    $data['pv_order_info'] = ($id) ? $this->pv_order_model->get($id, TRUE) : '';
    $data['all_clientes'] = $this->db->get('tbl_cliente')->result();
    $data['all_sedes'] = $this->db->get('tbl_sedes')->result();
    $data['subview'] = $this->load->view('admin/pv_order/add_pv_cliente', $data, FALSE);
    $this->load->view('admin/_layout_modal', $data);
  ...
  public function save_pv_order($id = NULL)
  {
    ...
    if(!empty($this->input->post('cliente_id')) && !empty($this->input->post('sede_id'))){
        $data = [
  ...
  //****************** GUARDAR LA NUEVA ACTIVIDAD (INICIO) ******************//
  ...
  //****************** GUARDAR LA NUEVA ACTIVIDAD (FIN) ******************//

  public function activities_calendar($cliente_id = NULL, $area = NULL)
  {
    $data['title'] = 'Calendario - Actividades a realizar';
    $data['page'] = 'Calendario - Actividades a realizar';
