...
class Cotizacion extends Admin_Controller
{
  ...
  /* NUEVO ACTUALIZADO (INICIO) */
  public function save_cotizacion($id = null)
  {

    $dir =  "./uploads/cotizaciones/cotizaciones";
    if (!is_dir($dir)) {
      mkdir($dir, 0777);
    }
  ...
  /* GUARDAR EL PDF DE LA COTIZACIÓN AGREGADA */
  ...
  public function CotizacionList($actionURL = NULL, $type = null)
  {
    ...
    if ($this->input->is_ajax_request()) {
      ...
      foreach ($fetch_data as $_key => $document) {
        ...
        $sub_array[] = (in_array($this->session->userdata('designations_id'), [1, 2, 3])) ? display_money($document->monto) : '';
        ...
        $sub_array[] = $document->valorizacion_id;
        ...
        */
        ...
        $pdf_quotation = $document->pdf_filequotation;

        if($pdf_quotation != "" && $pdf_quotation != null){
            $cotizacion_documento = '<span data-placement="top" data-toggle="tooltip" title="DESCARGAR DOCUMENTO DE COTIZACIÓN" >
                                        <a target="_blank"  class="btn btn-success btn-xs"  href="' . base_url() . 'uploads/cotizaciones/cotizaciones/' . $pdf_quotation . '">
   ...
   * @author aQMiGuEL <email@email.com>
   */
  public function orden_trabajo($report = NULL)
  {
    $data['title'] = 'Orden de trabajo';
    $data['page'] = 'Orden de trabajo';
    if ($report == 'report') :
      $data['dt_buttons'] = TRUE;
      $data['subview'] = $this->load->view('admin/cotizaciones/orden_trabajo_report', $data, TRUE);
    else :
  ...
  public function cotizacion_orden_trabajo_report_list($type = NULL)
  {

    if ($this->input->is_ajax_request()) {
      $this->load->model('datatables');
      $this->datatables->select = 'ot.*, ser.service, cli.*, se.sede, des.designations, cot.*';
      $this->datatables->table = 'tbl_cotizacion_ot ot';
      $this->datatables->join_table = array('tbl_cotizaciones cot', 'tbl_cliente cli', 'tbl_sedes se', 'tbl_services ser', 'tbl_designations des');/*/*  */
      $this->datatables->join_where = array('ot.cotizacion_id = cot.cotizacion_id', 'cot.cliente_id = cli.cliente_id', 'cot.sede_id = se.sede_id', 'ser.service_id = cot.service_id', 'ot.area_asignada = des.designations_id');

      $this->datatables->column_search = array('cot.cotizacion_id', 'ot.cotizacion_ot_id', 'cli.ruc', 'cli.razon_social');
      $this->datatables->column_order = array('cot.cotizacion_id', 'ot.cotizacion_ot_id', 'cli.ruc', 'cli.razon_social');
      $this->datatables->order = array('ot.cotizacion_ot_id' => 'desc');

      $where = [];
      if( $this->db->where(['designations_id' => $this->session->userdata('designations_id') ])->get('tbl_designations')->row()->departments_id == 2 ):
        
        $where = ['ot.area_asignada' => $this->session->userdata('designations_id')] ;
      endif;
      ...
      /* if (!empty($type)) {
        ...
        // $where = null;
        // $where = array('cot.status>='=>'22', 'cot.accion' => 'OT');
        $where = array('cot.status>' => '2');
      } */
      // $where = NULL;
...
die(); */
      ...
      foreach ($fetch_data as $_key => $document) {
        ...
        $sub_array[] = $document->cotizacion_ot_id;
        $sub_array[] = $document->cotizacion_id;
        $sub_array[] = display_date($document->fecha);
        $sub_array[] = display_date($document->end_date);
        $sub_array[] = display_date($document->end_date);
        $sub_array[] = $document->service;
        $sub_array[] = $document->ruc . ' - ' . $document->razon_social;
        $sub_array[] = $document->sede;
        $sub_array[] = $document->designations;
        $sub_array[] = $document->valorizacion_id;
        $sub_array[] = display_money( $document->monto );

        $conformidad = $this->db->where(['cotizacion_id' => $document->cotizacion_id])->get('tbl_cotizacion_conformidad_servicio')->row();
        $sub_array[] = ( $conformidad ) ? '<h4><span class="label label-success">Realizada</span></h4>' : '<h4><span class="label label-warning">Por Realizar</span></h4>';


        $cotizacion_documento = '<span data-placement="top" data-toggle="tooltip" title="DESCARGAR DOCUMENTO DE COTIZACIÓN" >        
        <!--<a target="_blank"  class="btn btn-success btn-xs"  href="' . base_url() . 'admin/cotizacion/ot_pdf/' . $document->cotizacion_ot_id . '"><span class="fa fa-file"></span></a>-->
        <a target="_blank"  class="btn btn-success btn-xs"  href="' . base_url() . 'admin/cotizacion/pdf/' . $document->cotizacion_id . '"><span class="fa fa-file"></span></a>
        </span>';
...
echo "</pre>"; */
  ...
  public function pdf($id)
  {
    ...
    $title = 'COTIZACION N° ' . $info->cotizacion_id . ' - ' . date(strftime("Y", $info->fecha));
