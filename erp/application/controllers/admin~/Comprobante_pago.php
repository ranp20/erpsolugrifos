    ...
    if ($this->input->is_ajax_request()) {
      ...
      foreach ($fetch_data as $_key => $document) {
        ...
        <a target="_blank"  class="btn btn-success btn-xs"  href="' . base_url() . 'admin/cotizacion/pdf/' . $document->cotizacion_id . '"><span class="fa fa-file-pdf-o"></span></a>
          ...
          $upload_comprobantes = '<span data-placement="top" data-toggle="tooltip" title="COMPROBANTE DE PAGOS"><a  data-toggle="modal" data-target="#myModal"  class="btn btn-primary btn-xs" href="' . base_url() . 'admin/comprobante_pago/forms/comprobante_pago/' . $document->cotizacion_id . '"><i class="fa fa-upload"></i></a></span>';

          $upload_facturas = '<span data-placement="top" data-toggle="tooltip" title="FACTURA"><a  data-toggle="modal" data-target="#myModal"  class="btn btn-primary btn-xs" href="' . base_url() . 'admin/factura/form_facturas/' . $document->cotizacion_id . '"><i class="fa fa-upload"></i> F</a></span>';

          $detail_comprobante = '<span data-placement="top" data-toggle="tooltip" title="COMPROBANTE DE PAGOS"><a  data-toggle="modal" data-target="#myModal"  class="btn btn-primary btn-xs" href="' . base_url() . 'admin/comprobante_pago/detail_comprobante/' . $document->cotizacion_id . '"><i class="fa fa-list-alt"></i></a></span>';
  ...
  public function save_forms($form, $id)
  {
    if ($form == 'comprobante_pago') {
      ...
      if ($pendientes == 0) {
        ...
        $not_designations_id = 1;
        $not_message = 'Comprobantes Cubidos, Culminar Proceso';
        $data_cotizacion = [
          'status' => 33,
          'accion' => 'cierre',
          'area_actual' => 1
